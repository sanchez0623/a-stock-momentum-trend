...
- 回测中心五模块:持仓回测 / 信号审计 / 策略对比回测(同池同种子多风控变体消融,冷却×防守开关矩阵,回测区间可选+快捷选项,选股中心同源选池,单任务守卫防重复开跑,切页/刷新进度保持)/ 阶段·得分双维分桶因子回测 / K线缓存管理(新鲜度统计 + 增量补拉 + 回测快照预热)
...
| 回测中心 | 五模块:持仓回测 / 信号审计 / 策略对比回测(风控变体消融,同池同种子,区间快捷选项,单任务守卫,进度切页保持)/ 阶段·得分分桶 / K线缓存管理(增量补拉 + 快照预热),异步任务含 T+1 / 申报单位 / 风控闸门 |
