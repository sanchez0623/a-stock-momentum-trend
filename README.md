...
- 人话交易计划 + 手动回填成交 + 双写日志(CSV + SQLite);持仓操作时间线(建仓/加仓/减仓逐笔标注,清仓重开自动切分新周期)
...
- 回测中心五模块:持仓回测 / 信号审计 / 策略对比回测(同池同种子多风控变体消融,冷却×防守开关矩阵,回测区间可选+快捷选项,选股中心同源选池,单任务守卫防重复开跑,切页/刷新进度保持;每次运行自动落库——变体摘要+净值曲线+逐笔交易明细持久保存,「历史记录」子页可回看,明细支持按变体/股票筛选与跨变体对照,保留最近 20 次自动清理)/ 阶段·得分双维分桶因子回测 / K线缓存管理(新鲜度统计 + 增量补拉 + 回测快照预热)
...
| 自选与持仓 | 录入持仓(代码自动带出名称)、加仓、减仓、清仓;持仓操作时间线(建仓/加仓/减仓逐笔标注时间·数量·价格,减仓含已实现盈亏) |
...
| 回测中心 | 五模块:持仓回测 / 信号审计 / 策略对比回测(风控变体消融,同池同种子,区间快捷选项,单任务守卫,进度切页保持;结果+逐笔明细落库,历史可回看,按变体/股票筛选交易明细,保留最近 20 次)/ 阶段·得分分桶 / K线缓存管理(增量补拉 + 快照预热),异步任务含 T+1 / 申报单位 / 风控闸门 |
...
- **页面全部无响应 / 回测进度永远"等待进度上报" / 诊断与放弃按钮点了没反应**:后端 worker 进程大概率已原生崩溃(某个 C 扩展触发段错误 0xC0000005)。`uvicorn --reload` 的监督进程只在文件变化时重启,**不会重启已崩溃的 worker**,但继续占着端口,导致 TCP 能连上却永远无应答。处理:重启后端进程。已内置两层防护:① 启动时挂接 `faulthandler`,崩溃前会把 Python 调用栈打到 stderr 便于定位肇事扩展;② 前端任务轮询/诊断/放弃请求带 10 秒客户端超时,后端无响应时给出明确提示,且"放弃"本地立即生效不再被挂起的请求卡死
- 开发期建议:调试稳定后可用不带 `--reload` 的方式常驻运行,避免监督进程"占端口不干活"的假活状态
