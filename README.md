...
| 页面 | 说明 |
|------|------|
| 仪表盘 | 自选/持仓实时行情、风控状态、数据源健康、盈亏总览 |
| 选股 | 三因子全市场扫描(可组合 市场/板块/申万行业 缩小范围) |
| 自选与持仓 | 录入持仓(代码自动带出名称)、加仓、减仓、清仓 |
| 信号中心 | 单/多代码评估、持仓一键分析(持仓感知: 加仓/减仓/止损) |
| 交易计划 | 生成人话操作指引, 标记已执行/已忽略 |
| 交易日志 | 成交明细(双写 CSV+SQLite)、手动回填、CSV 导出 |
| 历史回顾 | 盈亏曲线/月度热力图/信号分布/单笔评分/健康度 |
| AI 复盘 | 规则诊断 + LLM 深度复盘(可接 DeepSeek/通义/Ollama) |
